...
public class ClaimQueryServiceImpl extends ServiceBaseImpl implements ClaimQueryService {
	...
	*/
	...
	public ResultPage<SimpleClaimVo> simpelClaimQuery(HashMap<String, Object> reqMap, PageParam pageParam) throws Exception {
		if(reqMap!=null && reqMap.size()>0){
			Page<SimpleClaimVo> claimVos = new Page<SimpleClaimVo>();
			Page<PrplClaim> claims = prplClaimMapper.findByPolicyInfo(pageParam, reqMap);
			...
			return Pages.convert(pageParam, claimVos, SimpleClaimVo.class);
		}else{
			return null;
		}
